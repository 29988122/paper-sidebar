...
這是一個專為 Chrome 原生**垂直分頁**設計的純配色主題，參考 Firefox 的淺色主題：淡灰色的分頁側欄、白色的網頁，目前分頁的圖示旁邊有一條小小的橘色標記。
...
- 分頁側欄淡灰（#F2F2F2），工具列和網頁外框更淡（#FAFAFA），網頁是白色，跟 Firefox 的淺色主題一樣，沒有偏藍的色調。
- 目前分頁是一顆較亮的膠囊，圖示旁有一條橘色（#FF9500）標記，高度跟圖示一樣。
- 分頁標題是黑色，其他分頁保持平整。
...
1. **作用中分頁一定使用 `toolbar` 顏色。** 同一個顏色也會畫在工具列、提示列和網頁外框上。如果把作用中分頁設成深色，工具列和外框也會跟著變深。這裡把 `toolbar` 設成 #FAFAFA、側欄設成 #F2F2F2，所以作用中分頁看起來是一顆稍亮的膠囊，網頁外框也維持淺色。
